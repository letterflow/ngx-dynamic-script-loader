...
export class NgxDynamicScriptLoader {
   ...
   */
  loadScripts(
    ...scriptRefs: NgxDynamicScriptRef[]
  ): Observable<NgxDynamicScriptLoaded[]> {
    const scriptRefs$ = scriptRefs.map((scriptRef) =>
      this.loadScript(scriptRef)
    );
  ...
  loadScript(
    ...
    return new Observable<NgxDynamicScriptLoaded>((subscriber) => {
      ...
      const onLoadCallback = (evt: Event) => {
        ...
        const result: NgxDynamicScriptLoaded = {
          fetched: true,
          loaded: true,
          name,
          src,
